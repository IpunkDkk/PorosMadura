...
export default async function CmsNewAdPage() {
  const { adSlots, media } = await getCmsAdsData()
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <h1 className="font-heading text-3xl font-black text-poros-navy">Iklan Baru</h1>
        </div>
      </div>
      <div className="rounded-xl border border-border-light bg-white p-6 shadow-sm">
        <AdForm adSlots={adSlots} mediaItems={media} action={saveAdAction} backHref="/cms/ads" />
      </div>
    </div>
  )
}
